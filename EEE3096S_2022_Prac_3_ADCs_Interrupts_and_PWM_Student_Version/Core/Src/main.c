...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  uint32_t adc_scaled = ADCtoCRR(adc_val);
	  sprintf(buffer, "ADC Value: %d | DC Value: %d/47999\r\n", adc_val, adc_scaled);
	  ...
	  // frequency may not change on button press because it is pressed while executing the delay function
